...
1. **Metadata header** (IDE settings like `NumberOfModules=`, `Library1=`, `@EndOfDesignText@`) — stripped by `get_file_content`, preserved by editing tools.
2. **Source code section** (after `@EndOfDesignText@`) — this is what `get_file_content` returns.
...
- **`analyze_module`**
  Analyzes a SINGLE .bas, .b4a, or .b4j module file: lists every Sub (name, parameters, return type, public/private, event handler detection), every Type declaration, and Globals presence. Also reports structural parse issues without compiling.
